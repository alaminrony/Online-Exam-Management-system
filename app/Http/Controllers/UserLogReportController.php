...
//        Helper::dump($request->all());
        $targetArr = [];
        $userList = ['' => __('label.SELECT_USER_OPT')] + User::withTrashed()->select(DB::raw("CONCAT(users.first_name,' ',users.last_name) as user_name"), 'users.id as user_id')
        ...
        if ($request->view == 'print') {
            return view('logReport.userLogReport.printUserLogReport')->with(compact('targetArr','request','userList'));
        } else if ($request->view == 'pdf') {
            $pdf = PDF::loadView('logReport.userLogReport.printUserLogReport', compact('targetArr','request','userList'))
                    ->setPaper('a4', 'portrait')
                    ->setOptions(['defaultFont' => 'sans-serif']);
            return $pdf->download('userLogReport.pdf');
        } else if ($request->view == 'excel') {
            $viewFile = 'logReport.userLogReport.printUserLogReport';
            $downLoadFileName = 'userLogReport.xlsx';
        ...
        return view('logReport.userLogReport.userLog')->with(compact('targetArr', 'request', 'userList'));
    }
    ...
    public function generate(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('userLogReport?' . $url)->withErrors($validator);
        }
        return redirect('userLogReport?generate=true&' . $url);
    }
