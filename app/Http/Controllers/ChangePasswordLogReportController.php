...
class ChangePasswordLogReportController extends Controller {

    private $controller = 'ChangePasswordLogReport';
...
//        Helper::dump($request->all());
                ...
                foreach ($totalLogArr as $date => $userDataArr) {
                    foreach ($userDataArr as $userId => $userDetailsArr) {
                        foreach ($userDetailsArr as $uniquKey => $logData) {
                            ...
                            $targetArr[$i]['affected_user_id'] = $logData['user_id'];
        ...
        if ($request->view == 'print') {
            return view('logReport.changePasswordLogReport.printChangePasswordReport')->with(compact('targetArr','request','userList'));
        } else if ($request->view == 'pdf') {
            $pdf = PDF::loadView('logReport.changePasswordLogReport.printChangePasswordReport', compact('targetArr','request','userList'))
                    ->setPaper('a4', 'portrait')
                    ->setOptions(['defaultFont' => 'sans-serif']);
            return $pdf->download('changePassword.pdf');
        } else if ($request->view == 'excel') {
            $viewFile = 'logReport.changePasswordLogReport.printChangePasswordReport';
            $downLoadFileName = 'changePassword.xlsx';
        ...
        return view('logReport.changePasswordLogReport.changePasswordLog')->with(compact('targetArr', 'request', 'userList'));
    }
    ...
    public function generate(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('changePasswordLog?' . $url)->withErrors($validator);
        }
        return redirect('changePasswordLog?generate=true&' . $url);
    }
