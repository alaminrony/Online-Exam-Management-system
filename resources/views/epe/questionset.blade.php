...
<div class="col-md-12">
    ...
    <div class="portlet light bordered">
            ...
            <div class="table-toolbar">
                <div class="note note-success">
                    <div class="row">
                        <div class="col-md-9 col-md-offset-1 text-center">
                            <address class="text-center">
                                <strong>{{__('label.SUBJECT')}}: </strong> {{$epeInfo->subject_title}}
                            </address>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6">
                                <div class="pull-left">
                                    <strong> {{__('label.TIME')}} : </strong>{{$epeInfo->obj_duration_hours.':'.$epeInfo->obj_duration_minutes.':00'}}
                                </div>
                            </div>

                            <div class="col-md-6 fixed_2"> 
                                <div class="pull-right">
                                    <strong>{{__('label.SELECTED_QUESTION')}} : </strong><span id="selectque">{{ $alreadySelected}}</span> {{__('label.OUT_OF').$epeInfo->obj_no_question}}
                                    <div class="pull-right" id="details" style="display:none;"><strong>&nbsp;{{__('label.SHOW_DETAILS')}}</strong></div>
                                    <div id="qusTypeDiv" style="display:none;">
                                        @foreach($questionTypeList as  $typeId =>$typeName)
                                        <strong>{{!empty($questionTypeList[$typeId])?$questionTypeList[$typeId]:0}} : </strong> <span id="selectqueFixed-{{$typeId}}">{{!empty($alreadySelectArr[$typeId])?$alreadySelectArr[$typeId]:0}}</span> Out of <span id="typeWiseTotal-{{$typeId}}">{{!empty($epeQusTypeList[$typeId])?$epeQusTypeList[$typeId]:0}} </span><br />
                                        @endforeach
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            @foreach($questionTypeList as  $typeId =>$typeName)
                            <strong>{{!empty($questionTypeList[$typeId])?$questionTypeList[$typeId]:0}} : </strong> <span id="selectque-{{$typeId}}">{{!empty($alreadySelectArr[$typeId])?$alreadySelectArr[$typeId]:0}}</span> Out of {{!empty($epeQusTypeList[$typeId])?$epeQusTypeList[$typeId]:0}}
                            @endforeach
                        </div>
                    </div>
                </div>
                    ...
                    <tr class="odd gradeX">
                        <td>
                            <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                <input name="question_id[{{$question->id}}]" type="checkbox" data-id="{{$question->id}}" data-sl-no="{{$sl}}" data-qus-type-id="{{$question->type_id}}" class="checkboxes {{$class}} {{!empty($classArr[$question->type_id])?$classArr[$question->type_id]:''}}" <?php echo $checked; ?> value="{{$question->id}}" />
                                <span></span>
                            </label>
                        </td>
                        <td>
                            <?php $slNo = ++$sl ?>
                            {{$slNo}}<input type="hidden" name="sl_no[{{$question->id}}]" value="{{$slNo}}"/></td>
                        <td> {{$question->name}} </td>
                        ...
                        <td>
                            <div class="input-icon">
                                ...
                                <input type="text" name="time[{{$question->id}}]" class="form-control timepicker w-100" data-id="{{$question->id}}" value="{{!empty($question->time)?$question->time:''}}" readonly="readonly" />
                            </div>
                        </td>
                          @endif
                        <td>
                            <div class="input-group">
                                <input type="text" name="mark[{{$question->id}}]" class="form-control w-100 integer-decimal-only" data-id="{{$question->id}}"  value="{{!empty($question->mark)?$question->mark:''}}" />
                            </div>
                        </td>
                    </tr>
                    @endforeach
        ...
        <div class="form-actions">
            <div class="row">
                <div class="text-center col-md-12 fixed_3">
                    <button type="submit" class="btn btn-circle green" id="questionSetSubmit"><i class="fa fa-save"></i> {{__('label.SAVE')}}</button>
                    <a href="{{URL::to('epe')}}">
        ...
        <input type="hidden" name="epe_id" value="{{$epeInfo->id}}"/>
        <input type="hidden" name="total_noque" id="currentv" value="{{$epeInfo->obj_no_question}}"/>
        {{ Form::close() }}
    </div>
                ...
                cancelButtonText: 'No, I do not agree',
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $.ajax({
                                url: "{{ URL::to('epe/updatedQuestionSet') }}",
                                type: "POST",
                                data: sData + "&" + nData,
                                dataType: 'json',
                                headers: {
