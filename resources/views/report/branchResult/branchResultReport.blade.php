...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-bookmark"></i>@lang('label.BRANCH_RESULT')
            </div>
            @if(!empty($request->generate) && $request->generate == 'true')
            @if(!empty($targetArr))
            <div class="pull-right" style="margin-top: 3px;" id="groupIcon">
                <a href="{{URL::to('branchResult?generate=true&from_date='.Request::get('from_date').'&to_date='.Request::get('to_date').'&branch_id='.Request::get('branch_id').'&view=print')}}"  title="view Print" target="_blank" class="btn btn-info tooltips rounded-0 print"><i class="fa fa-print"></i></a>
                <a href="{{URL::to('branchResult?generate=true&from_date='.Request::get('from_date').'&to_date='.Request::get('to_date').'&branch_id='.Request::get('branch_id').'&view=pdf')}}"  title="Download PDF File" class="btn btn-warning tooltips rounded-0 pdf"><i class="fa fa-file-pdf-o"></i></a>
                <a href="{{URL::to('branchResult?generate=true&from_date='.Request::get('from_date').'&to_date='.Request::get('to_date').'&branch_id='.Request::get('branch_id').'&view=excel')}}"  title="Download Excel" target="_blank" class="btn btn-danger tooltips rounded-0"><i class="fa fa-file-excel-o"></i></a>
            </div>
            @endif
            @endif
        </div>
        <div class="portlet-body">
            {!! Form::open(array('group' => 'form', 'url' => 'branchResult/generate','class' => 'form-horizontal')) !!}
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label class="control-label col-md-4" for="branchId">@lang('label.BRANCH'):<span class="text-danger">*</span></label>
                        <div class="col-md-8">
                            {!! Form::select('branch_id',$branchList,Request::get('branch_id'),['class' => 'form-control js-source-states','id'=>'branchId']) !!}
            ...
            <div class="bg-blue-hoki bg-font-blue-hoki">
                <h5 style="padding: 10px;">
                    @lang('label.BRANCH') : <strong>{{!empty($branchList[$request->branch_id]) ? $branchList[$request->branch_id] : 'N/A'}} |</strong>
