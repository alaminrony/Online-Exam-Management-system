...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-bookmark"></i>@lang('label.LOGIN_REPORT')
            </div>
            @if(!empty($request->generate) && $request->generate == 'true')
            <div class="pull-right" style="margin-top: 3px;" id="groupIcon">
                <a href="{{URL::to('loginReport?generate=true&from_date='.Request::get('from_date').'&to_date='.Request::get('to_date').'&view=print')}}"  title="view Print" target="_blank" class="btn btn-info tooltips rounded-0 print"><i class="fa fa-print"></i></a>
                <a href="{{URL::to('loginReport?generate=true&from_date='.Request::get('from_date').'&to_date='.Request::get('to_date').'&view=pdf')}}"  title="Download PDF File" class="btn btn-warning tooltips rounded-0 pdf"><i class="fa fa-file-pdf-o"></i></a>
                <a href="{{URL::to('loginReport?generate=true&from_date='.Request::get('from_date').'&to_date='.Request::get('to_date').'&view=excel')}}"  title="Download Excel" target="_blank" class="btn btn-danger tooltips rounded-0"><i class="fa fa-file-excel-o"></i></a>
            </div>
            @endif
        </div>
        ...
        <div class="portlet-body">
            {!! Form::open(array('group' => 'form', 'url' => 'loginReport/generate','class' => 'form-horizontal')) !!}
            <div class="row">
                <div class="col-md-4">
                ...
                <table class="table table-bordered table-hover">
                    <thead>
                        <tr class="center">
                            ...
                            <th>@lang('label.LOGIN_DATETIME')</th>
                            <th>@lang('label.LOGOUT_DATETIME')</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if(!empty($targetArr))
                        ...
                        <tr>
                            ...
                            <td>{{Helper::formatDateTime($target['login_datetime'])}}</td>
                            <td>{{Helper::formatDateTime($target['logout_datetime'])}}</td>
