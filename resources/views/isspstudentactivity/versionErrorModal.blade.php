<div class="modal-content">
    ...
    <div class="modal-body">
        ...
        <table class="table table-striped table-bordered table-hover">
            <tr>
                <th class="text-center" colspan="3"> {{__('label.SUPPORTED_BROWSER')}}</th>
            </tr>
            <tr>
                <th width="40%">{{__('label.SL')}} </th>
                <th width="40%">{{__('label.BROWSER')}} </th>
                <th width="40%">{{__('label.VERSION')}} </th>
            </tr>
            @php $i=0; @endphp
            @foreach($suppBrowser as $browserInfo)
            @php $i++; @endphp
            <tr>
                <td width="40%">{{$i}} </td>
                <td width="40%">{{$browserInfo['browser']}} </td>
                <td width="40%">{{$browserInfo['version']}} </td>
            </tr>
            @endforeach
        </table>
    </div>
