<h5>
    {{trans('english.SUBJECT')}} : <strong>{{$taeObjArr->subject->title}} |</strong> 
    {{trans('english.SUBMISSION_DATELINE')}} : <strong>{{$taeObjArr->deadline}} |</strong>
    {{trans('english.RESULT_PUBLISH_DATE_TIME')}} : <strong>{{!empty($taeObjArr->result_publish) ? $taeObjArr->result_publish: 'NOT SET'}}</strong>
</h5>
    ...
    <tbody>
        ...
        <tr class="contain-center">
            ...
            <td class="remove_action_{{$value->id}} text-center">
                @if($value->status == '0')
                <a data-tooltip="tooltip" data-toggle="modal" data-target="#show-modal" data-student_id="{{$value->id}}" data-status="2" href="#show-modal" class="btn green btn-sm specialpermission-tae-model tooltips" title="{{trans('english.CC_TAKEN')}}" data-container="body" data-trigger="hover" data-placement="top"> <i class="fa fa-edit" aria-hidden="true"></i></a>

                <a data-tooltip="tooltip" data-toggle="modal" data-target="#show-modal" data-student_id="{{$value->id}}" data-status="3" href="#show-modal" class="btn red-thunderbird btn-sm specialpermission-tae-model tooltips" title="{{trans('english.ABSENT')}}" data-container="body" data-trigger="hover" data-placement="top"> <i class="fa fa-edit" aria-hidden="true"></i></a>
                @endif
                @if($value->status == '2')
                <a data-tooltip="tooltip"  data-student_id="{{$value->id}}" data-status="2" href="#" class="taeUndoToken btn green-sharp btn-sm specialpermission-tae-model tooltips" title="{{trans('english.UNDO_CC_TAKEN')}}"> <i class="fa fa-mail-reply"></i></a>
                @elseif($value->status == '3')
                <a data-tooltip="tooltip"  data-student_id="{{$value->id}}" data-status="3" href="#" class="taeUndoToken btn btn-danger btn-sm specialpermission-tae-model tooltips" title="{{trans('english.UNDO_ABSENT')}}"> <i class="fa fa-mail-reply"></i></a>
                @endif
...
<script type="text/javascript">
    ...
    //tae undo token
    $(document).on('click', '.taeUndoToken', function (e) {
        e.preventDefault();
